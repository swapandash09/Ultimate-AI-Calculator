    ...
    bmCatalogList.innerHTML = bmCatalog.map((item, i) => {
      ...
      <li>
        <div class="bm-item-info">
          ...
          <span class="bm-item-qty">${item.qty} ${item.unit} = <span class="bm-editable-price" data-index="${i}" title="${currentLangText("tapToEdit")}">${fmtMoney(item.sell)}</span> (GST ${item.gst}%)${mrpNote}${margin}</span>
        </div>
...
bmCatAddBtn.addEventListener("click", () => {
  ...
  bmCatalog.push({ name, qty, unit, cost: cost || 0, sell, mrp: mrp || 0, gst });
  saveBmCatalog();
  renderBmCatalog();
...
function buildInvoiceHtml() {
  ...
  const rowsHtml = bmItems.map(item => `
    <tr>
      ...
      <td style="padding:8px 6px;border-bottom:1px solid #eee;text-align:center;">₹${item.rate.toFixed(2)}${item.mrp > 0 && Math.abs(item.mrp - item.rate) > 0.01 ? `<br><span style="font-size:9px;color:#999;">MRP ₹${item.mrp.toFixed(2)}</span>` : ""}</td>
...
bmGeneratePdfBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    bmItems.forEach(item => {
      const hasMrp = item.mrp > 0 && Math.abs(item.mrp - item.rate) > 0.01;
      ...
      if (hasMrp) {
        ...
        doc.text(`MRP Rs.${item.mrp.toFixed(2)}`, 112, y + 3.5);
        doc.setTextColor(0, 0, 0);
        doc.setFontSize(8.5);
        y += 3.5;
      }
