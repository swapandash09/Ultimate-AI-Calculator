    ...
    bmCatalogList.innerHTML = bmCatalog.map((item, i) => {
      ...
      <li>
        <div class="bm-item-info">
          ...
          <span class="bm-item-qty">${item.qty} ${item.unit} = ${fmtMoney(item.sell)} (GST ${item.gst}%)${margin}</span>
        </div>
  ...
  if (bmItems.length === 0) {
    ...
    bmItemList.innerHTML = bmItems.map((item, i) => {
      ...
      <li>
        <div class="bm-item-info">
          <span class="bm-item-name">${item.name}</span>
          <span class="bm-item-qty">${item.qty} ${item.unit} × ${fmtMoney(item.rate)}${discNote} · GST ${item.gst}%</span>
        </div>
...
bmGeneratePdfBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    bmItems.forEach(item => {
      ...
      doc.text(`${item.gst}%`, 125, y);
      doc.text(item.discount > 0 ? `${item.discount}%` : "-", 150, y);
