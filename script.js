...
const I18N = {
  en: {
    ...
    udhaarMarkedPaid: "Marked as paid",
  ...
  hi: {
    ...
    udhaarMarkedPaid: "चुकाया गया मार्क किया",
  ...
  bn: {
    ...
    udhaarMarkedPaid: "পরিশোধিত চিহ্নিত হয়েছে",
    ...
    bmUdhaarList.innerHTML = bmUdhaar.map((entry, i) => `
      <li class="${entry.paid ? "bm-udhaar-paid" : ""}">
        <div class="bm-item-info">
          ...
          <span class="bm-item-qty">${entry.note ? entry.note + " · " : ""}${new Date(entry.date).toLocaleDateString()}</span>
        </div>
        <span class="bm-item-price">${fmtMoney(entry.amount)}</span>
        ${!entry.paid ? `<button class="bm-udhaar-paid-btn" data-index="${i}" title="Mark Paid">✅</button>` : ""}
        <button class="bm-remove-btn" data-index="${i}" title="Remove">✕</button>
      </li>
    `).join("");
  }

  const outstanding = bmUdhaar.filter(e => !e.paid).reduce((sum, e) => sum + e.amount, 0);
  bmUdhaarGrandTotal.textContent = fmtMoney(outstanding);

  bmUdhaarList.querySelectorAll(".bm-udhaar-paid-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      bmUdhaar[parseInt(btn.dataset.index, 10)].paid = true;
      saveBmUdhaar();
      renderUdhaar();
      showToast(currentLangText("udhaarMarkedPaid"), "success");
    });
  });
...
bmUdhaarAddBtn.addEventListener("click", () => {
  ...
  bmUdhaar.unshift({ customer, amount, note: bmUdhaarNote.value.trim(), date: new Date().toISOString(), paid: false });
  saveBmUdhaar();
  renderUdhaar();
