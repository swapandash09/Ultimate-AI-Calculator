...
// Smart Bill Scanner with Enhanced Number Detection
...
function preprocessImage(img) {
    ...
    // Apply basic thresholding for contrast
    const thresholdData = ctx.getImageData(0, 0, width, height);
    const threshold = 128;
    for (let i = 0; i < thresholdData.data.length; i += 4) {
        const avg = thresholdData.data[i];
        ...
        img.onload = async () => {
            try {
                ...
                await worker.setParameters({
                    ...
                    user_defined_dpi: '200', // Higher DPI for better accuracy
                    // Removed tessedit_char_whitelist to capture more context
                });
                ...
                const cleanedText = text
                    .replace(/[^a-zA-Z0-9\s₹$.:]/g, '') // Remove special characters except currency and numbers
                    .replace(/\s+/g, ' ') // Normalize spaces
                    .trim();
                console.log('Cleaned OCR text:', cleanedText);

                // Enhanced regex for number detection
                const totalMatch = cleanedText.match(/(?:Total|TOTAL|Amount|AMOUNT|Sum|SUM|Final|Grand Total|Bill|Invoice|Net|Payable)[:\s]*(?:[₹$€£¥]?[\s]*)?(\d{1,3}(?:[.,]\d{3})*(?:\.\d{1,2})?)/i) ||
                                  cleanedText.match(/(?:[₹$€£¥]?[\s]*)?(\d{1,3}(?:[.,]\d{3})*(?:\.\d{1,2})?)(?:[\s]*$|[\s]+[^\d])/i);
                let total = totalMatch ? totalMatch[1] : null;

                // Clean the detected total (remove commas, ensure proper decimal)
                if (total) {
                    total = total.replace(/,/g, ''); // Remove thousands separators
