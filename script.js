...
bmGeneratePdfBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (bmShopProfile.qr) {
      try {
        doc.addImage(bmShopProfile.qr, "PNG", 172, 11, 22, 22);
        doc.setFontSize(6.5);
        doc.setFont(undefined, "bold");
        doc.setTextColor(255, 255, 255);
    ...
    if (structuredItems.length > 0) {
      structuredItems.forEach(li => {
        ...
        if (existing) {
          ...
          bmCatalog.push({ name: li.name, qty: li.qty, unit: "pcs", cost: 0, costEstimated: false, costFormula: null, sell: 0, mrp: li.mrp, gst: guessGstForName(li.name) || 5 });
        }
      });
    } else {
