...
// Calculator Logic with Enhanced Expression Handling
...
let lastClickTime = 0;
const MAX_INPUT_LENGTH = 100; // Maximum characters allowed in an expression
    ...
    buttons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (value === 'C') {
                ...
                if (!sanitizedInput) {
                    console.error('Invalid expression due to unbalanced parentheses:', currentInput);
                    currentInput = 'Error';
                ...
                try {
                    // Break down long expressions into smaller chunks if needed
                    let result;
                    if (sanitizedInput.length > 50) {
                        console.log('Breaking down long expression:', sanitizedInput);
                        // Split by top-level operators (not inside parentheses)
                        let parts = [];
                        let currentPart = '';
                        let parenLevel = 0;
                        for (let i = 0; i < sanitizedInput.length; i++) {
                            let char = sanitizedInput[i];
                            if (char === '(') parenLevel++;
                            if (char === ')') parenLevel--;
                            currentPart += char;
                            if ((char === '+' || char === '-') && parenLevel === 0 && i !== sanitizedInput.length - 1) {
                        ...
                        for (let part of parts) {
                            let subResult = math.evaluate(part);
                            result = char === '+' ? result + subResult : result - subResult;
                        }
                        result = result.toString();
                    } else {
                        result = math.evaluate(sanitizedInput).toString();
                    }
...
// Simplified image preprocessing for number detection
        ...
        img.onload = async () => {
            try {
                ...
                await worker.setParameters({
                    tessedit_pageseg_mode: Tesseract.PSM.AUTO, // Auto page segmentation for better number detection
                    user_defined_dpi: '600', // Higher DPI for accuracy
                    tessedit_char_whitelist: '0123456789', // Restrict OCR to numbers only
                });
                ...
                // Filter numbers to ensure they are valid (e.g., exclude very large or negative numbers)
                const filteredNumbers = numbers
                    .map(num => parseInt(num))
                    .filter(num => num >= 0 && num <= 9999); // Reasonable range for retailer use
