...
// Smart Bill Scanner with Enhanced Accuracy
...
function preprocessImage(img) {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const idx = (y * width + x) * 4;
            const avg = data[idx];
            const threshold = 128; // Dynamic thresholding could be added here
    ...
    billInput.addEventListener('change', async (event) => {
        ...
        img.onload = async () => {
            try {
                ...
                if (confidence < 60) {
                    console.warn('Low OCR confidence, scan may be inaccurate');
                ...
                // Fallback: Look for any large number in the last few lines (common for totals)
                if (!total) {
                ...
                // Clean and validate the detected total
                if (total) {
                    total = total.replace(/[.,]/g, match => match === ',' ? '' : '.'); // Normalize separators
                    total = parseFloat(total).toFixed(2); // Ensure two decimal places
                    console.log('Processed total:', total);

                    // Validate total (e.g., should be a reasonable bill amount)
                    const totalValue = parseFloat(total);
                    if (totalValue < 0 || totalValue > 1000000) { // Arbitrary range for a bill
