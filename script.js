...
function renderBmCatalog() {
  if (bmCatalog.length === 0) {
    ...
    bmCatalogList.innerHTML = bmCatalog.map((item, i) => {
      ...
      const mrpNote = item.mrp > 0 ? ` · MRP ${fmtMoney(item.mrp)}` : "";
      return `
      <li>
        <div class="bm-item-info">
          <span class="bm-item-name">${item.name}</span>
          <span class="bm-item-qty">${item.qty} ${item.unit} = <span class="bm-editable-price" data-index="${i}" title="${currentLangText("tapToEdit")}">${fmtMoney(item.sell)}</span> (GST ${item.gst}%)${mrpNote}${costNote}</span>
        </div>
...
function previewCatCost() {
  if (bmCatCostTouched) return;
  const mrp = validNumber(bmCatMrp.value);
  const margin = validNumber(bmCatMargin.value);
  if (mrp === null || mrp <= 0) return;
  const divisor = margin !== null && margin > 0 ? 1 + margin / 100 : 1.09;
  bmCatCost.value = (mrp / divisor).toFixed(2);
}
...
bmCatAddBtn.addEventListener("click", () => {
  ...
  // if you know your margin %, Cost = MRP ÷ (1 + margin/100) — exact, using
  // your real number. Only if neither is given do we fall back to a rough
  // ~9% estimate (MRP ÷ 1.09), common for FMCG/baby-food style products.
  let cost = manualCost;
  ...
  bmCatalog.push({ name, qty, unit, cost: cost || 0, costEstimated, sell, mrp: mrp || 0, gst });
  saveBmCatalog();
  renderBmCatalog();
...
const NUMBER_WORDS = {
  ...
  "चालीस": 40, "पचास": 50, "सौ": 100, "आधा": 0.5, "आधी": 0.5,
  ...
  "পঞ্চাশ": 50, "একশ": 100, "আধা": 0.5
};

function wordsToDigits(text) {
  // Token-based, not regex-with-\b — \b doesn't reliably work across word
  // boundaries in Devanagari/Bengali script in JS regex (it's ASCII-aware
  // only), which was silently breaking every Hindi/Bengali number word.
  return text.split(/\s+/).map(tok => {
    const clean = tok.toLowerCase().replace(/[.,!?।]+$/, "");
    return Object.prototype.hasOwnProperty.call(NUMBER_WORDS, clean) ? NUMBER_WORDS[clean] : tok;
  }).join(" ");
}
...
function processVoiceBillCommand(rawTranscript) {
  ...
  if (numbers.length >= 2) {
    ...
    if (resWith.tier > resWithout.tier) {
      qty = 1; // the number belongs to the product name
  ...
  const { item: match } = findCatalogItemByName(spokenName, bmCatalog);
  ...
  let unit = spokenUnit || (match.unit === "pcs" ? "pcs" : match.unit);
  ...
  let mrp = 0;
  if (match.mrp > 0) {
    const mrpPerUnit = getPerUnitRate({ ...match, sell: match.mrp }, unit);
    if (mrpPerUnit !== null) mrp = mrpPerUnit * qty;
  }

  const amount = qty * rate;
  bmItems.push({ name: match.name, qty, unit, rate, mrp, gst: match.gst, discount: 0, amount });
  saveBmItems();
  renderBmItems();
...
bmVoiceAddBtn.addEventListener("click", () => {
  ...
  recognition.onresult = (event) => {
    processVoiceBillCommand(event.results[0][0].transcript);
  };
  recognition.onerror = () => {
    bmVoiceAddStatus.className = "error";
...
function selectUnit(unit) {
  ...
  if (bmMatchedCatalogItem) {
    ...
    if (bmMatchedCatalogItem.mrp > 0) {
      const mrpPerUnit = getPerUnitRate({ ...bmMatchedCatalogItem, sell: bmMatchedCatalogItem.mrp }, unit);
      if (mrpPerUnit !== null) bmItemMrp.value = mrpPerUnit.toFixed(2);
    }
  }
  updateLiveAmount();
}
...
bmItemName.addEventListener("input", () => {
  ...
  if (match) {
    ...
    if (match.mrp > 0) {
      const mrpPerUnit = getPerUnitRate({ ...match, sell: match.mrp }, bmSelectedUnit);
      if (mrpPerUnit !== null) bmItemMrp.value = mrpPerUnit.toFixed(2);
    } else {
      bmItemMrp.value = "";
    }
...
function addBmItem() {
  ...
  // MRP: prefer whatever's actually in the field (shopkeeper can type or
  // override it directly at billing time — the real printed MRP on a
  // product can differ from what's saved in the catalog). Falls back to
  // the catalog-derived, unit-converted value only if the field is empty.
  let mrp = 0;
  ...
  if (manualMrp !== null && manualMrp > 0) {
    ...
    const mrpPerUnit = getPerUnitRate({ ...bmMatchedCatalogItem, sell: bmMatchedCatalogItem.mrp }, bmSelectedUnit);
    if (mrpPerUnit !== null) mrp = mrpPerUnit * qty;
  }
  bmItems.push({ name, qty, unit: bmSelectedUnit, rate, mrp, gst, discount, amount });
  saveBmItems();
  renderBmItems();
...
bmGeneratePdfBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    const colStartX = 108, colEndX = 178;
    const colPositions = {};
