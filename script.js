...
function findCatalogItemByName(spokenName, bmCatalog) {
  ...
  item = bmCatalog.find(i => i.name.toLowerCase().startsWith(n) || n.startsWith(i.name.toLowerCase()));
  if (item) return { item, tier: 2 };
  item = bmCatalog.find(i => i.name.toLowerCase().includes(n) || n.includes(i.name.toLowerCase()));
...
function stripUnitWord(text) {
  let rest = text;
  let spokenUnit = null;
  for (const [unit, words] of Object.entries(UNIT_SPEECH_WORDS)) {
...
function processVoiceBillCommand(transcript) {
