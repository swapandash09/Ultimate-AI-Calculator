...
function previewCatCost() {
  ...
  } else if (isNestleBabyFood(bmCatName.value)) {
    bmCatCost.value = nestleCostFromMrp(mrp).purchaseRate.toFixed(2);
  }
  // Regular grocery with no margin given: leave Cost Price blank — no
  // formula applies, entirely optional.
}
...
bmCatAddBtn.addEventListener("click", () => {
  ...
  if ((cost === null || cost <= 0) && mrp > 0) {
    ...
    } else if (isNestleBabyFood(name)) {
      cost = nestleCostFromMrp(mrp).purchaseRate;
      costEstimated = true;
      costFormula = "nestle";
    }
  }
  bmCatalog.push({ name, qty, unit, cost: cost || 0, costEstimated, costFormula, sell, mrp: mrp || 0, gst });
  saveBmCatalog();
  renderBmCatalog();
...
function updateLiveAmount() {
  ...
  if (bmMatchedCatalogItem && bmMatchedCatalogItem.cost > 0) {
    const costTotal = bmMatchedCatalogItem.cost * qty;
    const profit = afterDiscount - costTotal;
    const profitPct = costTotal > 0 ? (profit / costTotal) * 100 : 0;
    profitNote = ` · ${currentLangText("boughtAtLabel")} ${fmtMoney(costTotal)} · ${currentLangText("profitLabel")} ${profit >= 0 ? "" : "-"}${fmtMoney(Math.abs(profit))} (${profitPct.toFixed(1)}%)`;
  }
  bmLiveAmount.textContent = `= ${fmtMoney(afterDiscount)}${profitNote}`;
  bmLiveAmount.classList.toggle("bm-profit-negative", profitNote.includes("-"));
}
...
bmAutoFetchInput.addEventListener("change", async () => {
  ...
  try {
    ...
    // here as MRP), never a trustworthy selling price or GST rate. Each
    // needs its selling price set once before it's usable for billing.
    analysis.lineItems.forEach(li => {
      ...
      if (existing) {
        ...
        bmCatalog.push({ name: li.label, qty: 1, unit: "pcs", cost: 0, costEstimated: false, costFormula: null, sell: 0, mrp: li.amount, gst: guessGstForName(li.label) || 5 });
      }
    });
    saveBmCatalog();
    renderBmCatalog();
    ...
    bmAutoFetchStatus.textContent = currentLangText("autoFetchAddedToCatalog")(analysis.lineItems.length);
    showToast(currentLangText("autoFetchAddedToCatalog")(analysis.lineItems.length), "success");
  } catch (err) {
    console.error(err);
    bmAutoFetchStatus.className = "error";
