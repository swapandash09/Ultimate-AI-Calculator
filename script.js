...
function finalizeVoiceAdd(match, qty, unit) {
  ...
  const cost = match.cost > 0 ? match.cost * qty : 0;
...
function askForVoiceQuantity(match, unit) {
  ...
  speak(`${match.name}. ${question}`, I18N[currentLang].speech);

  const recognition = getSpeechRecognition();
  if (!recognition) return; // already confirmed supported by the caller's initial check

  // Give the spoken question a moment to finish before listening, otherwise
  // the mic can pick up the tail end of our own voice.
  setTimeout(() => {
...
function computeBillTotals() {
  ...
  if (hasCostData) {
    ...
    resultRows.push({
      label: `📊 ${currentLangText("totalProfitLabel")}`,
...
function addBmItem() {
  ...
  const cost = bmMatchedCatalogItem && bmMatchedCatalogItem.cost > 0 ? bmMatchedCatalogItem.cost * qty : 0;
