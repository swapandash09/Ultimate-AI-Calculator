...
bmStockPdfBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    doc.text(bmShopProfile.name || currentLangText("stockReportLabel"), 105, 15, { align: "center" });
    doc.setFontSize(9);
    doc.setFont(undefined, "normal");
    doc.text(`${currentLangText("stockReportLabel")} — ${new Date().toLocaleDateString()}`, 105, 22, { align: "center" });
...
const loadXlsxLib = makeLazyLoader("https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js");
...
bmStockExcelBtn.addEventListener("click", async () => {
  ...
  showToast(currentLangText("pdfGenerating"), "info");
  try {
    if (!window.XLSX) await loadXlsxLib();
    const rows = stockItems.map(item => ({
      Item: item.name,
      "Stock (PCS)": item.stock,
      MRP: item.mrp || item.sell,
      "Total Value": item.stock * (item.mrp || item.sell)
    }));
    const grandTotal = stockItems.reduce((sum, item) => sum + item.stock * (item.mrp || item.sell), 0);
    rows.push({ Item: "", "Stock (PCS)": "", MRP: "TOTAL", "Total Value": grandTotal });

    const ws = window.XLSX.utils.json_to_sheet(rows);
    ws["!cols"] = [{ wch: 28 }, { wch: 12 }, { wch: 10 }, { wch: 14 }];
    const wb = window.XLSX.utils.book_new();
    window.XLSX.utils.book_append_sheet(wb, ws, "Stock Report");
    window.XLSX.writeFile(wb, `Stock_Report_${Date.now()}.xlsx`);
    showToast(currentLangText("pdfGenerated"), "success");
  } catch (err) {
    console.error(err);
    showToast(currentLangText("pdfFailed"), "error");
