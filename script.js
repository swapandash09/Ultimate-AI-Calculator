...
// Calculator Logic with Improved Multi-Number Handling
...
function validateExpression(expr) {
    // Remove invalid characters, but allow numbers, operators, parentheses, and spaces
    ...
    buttons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (value === 'C') {
                ...
                try {
                    // Let math.js handle operator precedence directly unless the expression is extremely long
                    let result;
                    if (sanitizedInput.length > 50) {
                        console.log('Handling long expression:', sanitizedInput);
                        // For very long expressions, evaluate in chunks while respecting precedence
                        result = math.evaluate(sanitizedInput, { precedence: true }).toString();
                    } else {
