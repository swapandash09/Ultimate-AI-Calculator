...
function speak(text) {
    const translations = {
        'en-IN': {
            ...
            'Low confidence in scan, please try a clearer image': 'Low confidence in scan, please try a clearer image',
        ...
        'hi-IN': {
            ...
            'Low confidence in scan, please try a clearer image': 'स्कैन में कम विश्वास, कृपया एक स्पष्ट छवि आज़माएं',
        ...
        'bn-IN': {
            ...
            'Low confidence in scan, please try a clearer image': 'স্ক্যানে কম আস্থা, দয়া করে একটি পরিষ্কার ছবি চেষ্টা করুন',
...
// Smart Bill Scanner with Handwritten Support
const billInput = document.getElementById('billInput');
const billResult = document.getElementById('billResult');
const scanStatus = document.getElementById('scanStatus');

// Function to preprocess image for better OCR accuracy, especially for handwritten text
function preprocessImage(img) {
    ...
    const blurRadius = 1;
    const weights = [1/9, 1/9, 1/9, 1/9, 1/9, 1/9, 1/9, 1/9, 1/9]; // Simple 3x3 Gaussian kernel
    ...
    imageData = ctx.getImageData(0, 0, width, height);
    data = imageData.data;
    const contrastFactor = 2.0; // Increase contrast for handwritten text
    for (let i = 0; i < data.length; i += 4) {
    ...
    // Apply dilation to thicken handwritten text
    imageData = ctx.getImageData(0, 0, width, height);
    data = imageData.data;
    tempData = new Uint8ClampedArray(data.length);
    for (let y = 1; y < height - 1; y++) {
        for (let x = 1; x < width - 1; x++) {
            const idx = (y * width + x) * 4;
            let maxValue = 0;
            ...
            tempData[idx] = tempData[idx + 1] = tempData[idx + 2] = maxValue;
            tempData[idx + 3] = data[idx + 3];
        }
    }
    ctx.putImageData(new ImageData(tempData, width, height), 0, 0);
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const idx = (y * width + x) * 4;
            const avg = data[idx];
            const threshold = 100; // Lower threshold for handwritten text
        ...
        img.onload = async () => {
            try {
                ...
                if (confidence < 50) {
                    ...
                    scanStatus.textContent = translations[currentLang]['Low confidence in scan, please try a clearer image'] || 'Low confidence in scan, please try a clearer image';
                ...
                const pricePattern = /\b(\d{1,3}(?:[.,]\d{3})*(?:\.\d{1,2})?)\s*(?:RUPEES|RS|RPS|₹|$)/i;
                ...
                if (!total) {
                    ...
                    for (const line of lines) {
                        ...
                        if (match) {
                            let price = match[1].replace(/[.,]/g, match => match === ',' ? '' : '.');
                ...
                if (total) {
                    total = total.replace(/[.,]/g, match => match === ',' ? '' : '.');
