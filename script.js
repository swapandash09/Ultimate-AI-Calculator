...
// ---------- Bill Maker (grocery-style bill builder + PDF export) ----------
...
function saveBmItems() {
  localStorage.setItem("calc_bm_items", JSON.stringify(bmItems));
}
    ...
    bmItemList.innerHTML = bmItems.map((item, i) => `
      <li>
        <div class="bm-item-info">
          ...
          ${item.qty ? `<span class="bm-item-qty">${item.qty}</span>` : ""}
        </div>
        <span class="bm-item-price">${fmtMoney(item.price)}</span>
        <button class="bm-remove-btn" data-index="${i}" title="Remove">✕</button>
      </li>
    `).join("");
  }

  const total = bmItems.reduce((sum, item) => sum + item.price, 0);
...
function addBmItem() {
  ...
  const qty = bmItemQty.value.trim();
  const price = validNumber(bmItemPrice.value);
  ...
  bmItems.push({ name, qty, price });
  saveBmItems();
  renderBmItems();
  speak(`${name} ${price}`, I18N[currentLang].speech);
...
// jsPDF loads on demand too, same reasoning as the OCR engine — keep the
// app itself fast, only pull in the heavy library when actually needed.
...
bmGeneratePdfBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    doc.text("INVOICE", 105, 16, { align: "center" });

    doc.setFontSize(10);
    doc.setFont(undefined, "normal");
    doc.text(`Date: ${new Date().toLocaleDateString()}`, 14, 26);
    doc.text(`Bill To: ${name}`, 14, 33);
    doc.text(`Phone: ${phone}`, 14, 39);
    doc.text(`Address: ${address}`, 14, 45);
    ...
    bmItems.forEach(item => {
      ...
      doc.text(`Rs. ${item.price.toFixed(2)}`, 196, y, { align: "right" });
      y += 8;
    });
    ...
    doc.save(`Bill_${safeName}_${Date.now()}.pdf`);
